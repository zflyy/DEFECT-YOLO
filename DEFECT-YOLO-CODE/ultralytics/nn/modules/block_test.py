...
class MDCBS(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("x_cheap","*"*99,x_cheap.shape)
        x_group = rearrange(x_group, 'bs (g ch) h w -> bs ch h w g', g=self.groups)
        # print("x_group","*"*99,x_group.shape)

        x1=self.convs[0](x_group[..., 0])
        x2=self.convs[1](x_group[..., 1])
        # print("x1","*"*99,x1.shape)
        # print("x2","*"*99,x2.shape)
...
class ASPPE(nn.Module):
    ...
    def __init__(self, c1, c2, k=5):  # equivalent to SPP(k=(5, 7, 9, 11))
        super().__init__()
        c_ = c1 // 2  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
    ...
    def forward(self, x):
        ...
        # out=self.att(self.cv2(torch.cat((x, y1, y2,y3), 1)))
        out_1=self.att(self.cv2(torch.cat((x_1, y1, y2, y3,y4), 1)))
        out_2=self.cv3(x)
...
class MCA(nn.Module):
    ...
    def __init__(self, channels, factor=32) -> None:
        ...
        self.group = factor
        midc = max(8, channels // self.group)
        self.conv = Conv(channels, midc, 1)
    ...
    def forward(self, x):
        shortcut=x
        _, _, h, w = x.size()
        x_pool_h, x_pool_w, x_pool_ch = self.pool_h(x), self.pool_w(x).permute(0, 1, 3, 2), self.agp(x)
        x_pool_hw = torch.cat([x_pool_h, x_pool_w], dim=2)
        x_pool_hw = self.conv(x_pool_hw)

        x_pool_hw_branch_1 = self.conv_hw(x_pool_hw)
        x_pool_h, x_pool_w = torch.split(x_pool_hw_branch_1, [h, w], dim=2)
        ...
        return shortcut * x_pool_h.sigmoid() * x_pool_w.permute(0, 1, 3, 2).sigmoid() * x_pool_ch.sigmoid()
